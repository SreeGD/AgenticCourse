...
Stuffing everything into one system prompt makes the agent confused and expensive. **Multi-agent splits the system prompt across specialists**, each tuned for its role.
...
**The trade-off:** multi-agent is 2-3× more expensive but produces noticeably better output for tasks with distinct sub-skills. Use it when the task genuinely benefits from specialization.
...
| You want different models per role (Haiku for routing, Sonnet for writing) | One model handles it all |
...
| Cost is acceptable (2-3× single-agent) | Cost-sensitive |
...
A: No. **Multi-agent is 2-3× more expensive and harder to debug.** Use it only when the task has genuinely distinct sub-skills, or when you want to use different models per role (the cost-optimization angle). For 70% of production agents, well-prompted single-agent is enough.
...
**Q: How big can LTM get?**

A: Memory-bound (or DB-bound). For one user with 500 facts, total ~50KB of text + ~150KB of vectors = 200KB. Multiply by 1M users = 200GB. Real production LTMs use durable stores (pgvector, Pinecone, Weaviate) with TTL/archive policies for old facts.

**Q: Can LTM be shared across users (e.g., "things any user might ask about")?**

A: Yes — but at that point it's just RAG. The user_id filter is what makes it "long-term memory" (personal context). Drop the filter and it becomes shared knowledge.
