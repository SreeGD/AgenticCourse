...
Composes every primitive from the course into one architecture:
    ...
    user input
       ...
       │       ├── prompt=CACHED system    (long persona/rules — cache_control)
...
show tokens, cache hits, guard verdicts, and a final cost summary.
"""
...
# =====================================================================
# 3. THE CACHED SYSTEM PROMPT — long enough to be cacheable
# =====================================================================
...
- Q: "Show me how to use it in a FastAPI app."
  A: "You'd put the cache marker on the SystemMessage as shown in the tutorial (LEARNINGS.md), and inject it via a FastAPI dependency that returns the ChatAnthropic instance. Roughly: [code]. The caching is invisible to FastAPI — it happens inside the model call."
...
# =====================================================================
# 4. THE AGENT — memory + caching + tools
# =====================================================================
...
def cost_usd(in_tok, out_tok, cache_read, cache_create):
    """Sonnet 4.6 pricing: $3 input, $15 output, $0.30 cache_read, $3.75 cache_create."""
    fresh = max(0, in_tok - cache_read - cache_create)
    return (fresh * 3 + out_tok * 15 + cache_read * 0.3 + cache_create * 3.75) / 1_000_000
...
if total_in > 0:
    hit_rate = total_cread / total_in * 100
    print(f"\nCache hit rate (cache_read / total_in): {hit_rate:.1f}%")
    print(f"What a 4-turn conversation looks like in production:")
    print(f"  - {results[3][1]['refused'] and 'one' or 'no'} turn refused upfront (zero LLM cost)")
    print(f"  - rest hit the cache after turn 1")
