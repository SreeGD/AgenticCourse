...
This is why **memory + caching are a married pair**. `MemorySaver` makes the agent useful; caching ([lesson 04](04-prompt-caching.md)) keeps cost bounded.
...
1. **Continue Alice's conversation** — add `turn("alice", "What was the answer again?")`. Watch Claude recall `46` without re-running tools.
...
4. **Pair with caching** — add a substantial system prompt with `cache_control`. Watch input cost stop growing linearly. This is the *complete* production chatbot architecture.
...
A: Memory-bound until you cap it. Each AIMessage / ToolMessage is small (~hundreds of bytes), but a chatty conversation can hit thousands of messages and tens of thousands of tokens. **At that point you're paying for token re-sending, not memory.** Mitigate with summary memory or prompt caching.
...
- **Pairs with:** [04 — Prompt caching](04-prompt-caching.md) — memory makes input grow, caching keeps cost flat
