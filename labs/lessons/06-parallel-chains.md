...
| **Multi-model voting** | Same prompt to Claude + GPT + Gemini, take majority |
...
A: No hard limit, but at ~10+ branches you're rate-limiting Anthropic. Use exponential-backoff retries and consider Anthropic's Batches API for 10+ concurrent requests.
...
A: Not in $ — you make the same N calls either way. Parallel saves *time*. Cheaper happens when the LLM provider also batches your requests, but that's an Anthropic optimization, not LangChain's.
