...
```
phase            in    out   time       cost
─────────────────────────────────────────────
clarify        1154   1336  23.8s    $0.0235
decompose      2042    672  11.6s    $0.0162
implement      2574   2297  31.6s    $0.0422
verify         4010   1270  21.3s    $0.0311
─────────────────────────────────────────────
TOTAL          9780   5575  88.3s    $0.1130
```

**~$0.11 and ~90 seconds** for a fully-specified, decomposed, implemented, and verified ~7.5 KB Python program with 7 acceptance criteria.

Compare to vibe coding (Session 5 next): maybe $0.02 and 20s, but you have to test, iterate, and trust that the model thought of every edge case.

For prototypes: vibe wins. For production code: SDD wins by a lot. The ~5× cost buys an audit trail (spec.md, tasks.md, verification_report.md) that you can show a manager, a compliance reviewer, or future-you.
...
**Don't reach for SDD on trivial work.** The 90-second / $0.11 overhead is worth it for production code, but it's overkill for "rename this variable."
...
5. **Compare to vibe coding** (lesson 16 coming) — run the same intent through both; compare quality, cost, and time.
...
A: Add a "phase 3.5" that takes the spec's acceptance criteria and generates pytest tests for them. Run the tests in phase 4 instead of just reading the code. This is the **strongest version of SDD** — every criterion becomes a test that either passes or fails on the actual code.

**Q: Can I reuse the spec across multiple implementations?**

A: Yes — that's a common pattern. Generate one spec, then run phases 2-4 multiple times with different implementations (different languages, different libraries). The spec is the contract; implementations are interchangeable.

**Q: Cost-wise, is SDD always more expensive than vibe coding?**

A: Yes — 4 phases vs. ~1 phase. Roughly 5-10× the LLM cost for the same intent. But you save human time downstream (no debugging cycle, no "wait, what does this even do?", no compliance pushback). The total cost-to-shipped-correct-code is usually lower for SDD on non-trivial work.

**Q: How do I use SDD in real Claude Code workflows?**

A: Anthropic itself has tooling around this pattern (Claude Code "plans," `superpowers:spec-driven-development` skill, etc.). The technique generalizes: any time you'd write a design doc + tickets + implementation + review for a human, you can let an LLM agent do the same pipeline. The structure is what gives you predictability.

**Q: What about cost optimization? Can the planner be a smaller model?**

A: Yes. Use Haiku for clarify + decompose (cheap, structured), Sonnet for implement (code generation needs strength), Opus for verify (highest fidelity check). That mix-of-models pattern is standard in production SDD; cuts cost ~50% with no quality drop.
