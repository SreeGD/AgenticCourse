...
The loop runs until Claude stops asking for tools (i.e., produces an `AIMessage` with `.content` and no `tool_calls`).
...
A: Yes — by default Claude will ask for clarification when a tool's args aren't clear. You can also engineer prompts ("Ask me for missing info before calling tools") to bias this behavior.
...
Without a limit, a bug can run up your Anthropic bill.
