...
**Pattern note:** `Command(resume=...)` on a re-invocation of an already-resumed interrupt won't work — the resume value is sticky per thread. To **fork** with a different decision, use `update_state(as_node=...)` to write what the node would have produced and let the rest of the graph continue.
...
- "What would the system have inferred if Sonnet was used instead of Haiku at step 3?"
...
A graph can be a node inside another graph. Build a `summarize_doc` subgraph, then use it as a node in 10 different parent graphs. The state of the subgraph is sealed inside the node from the parent's perspective.

(Not in this demo — worth its own session.)
...
A: As long as the checkpointer's storage. `MemorySaver` dies with the process; `SqliteSaver` lives in a file; `PostgresSaver` lives in your DB. There's no LangGraph-imposed timeout. Production workflows that wait for human action can pause for days.

**Q: Can two humans see the same interrupt and race to resume?**

A: Last write wins by default. For approval workflows where only one decision should be accepted, add an outer lock (DB row, distributed lock) around the `Command(resume=...)` call.

**Q: How do I show progress to a user while the graph runs?**

A: Use `agent.stream_events()` (or `astream_events()` for async). It yields events at every node entry/exit/error. Build a UI that subscribes to those events and shows "currently in: research" / "moved to: draft" / "interrupted at: human_review."

**Q: What if I want some nodes to run in parallel?**

A: `add_edge(START, "node_a")` + `add_edge(START, "node_b")` runs both at the same time when both have START as a predecessor. The state merging behavior is configurable per channel. (Beyond this lesson — worth its own deep dive.)

**Q: Can custom graphs use the same checkpointer as `create_react_agent`?**

A: Yes — same `Checkpointer` interface. Pass `MemorySaver()` / `SqliteSaver(...)` / `PostgresSaver(...)` to `.compile(checkpointer=...)`. The checkpointer doesn't know whether you wrote the graph manually or used `create_react_agent`.

**Q: Is custom LangGraph more expensive than `create_react_agent`?**

A: Per LLM call, no. Graphs add zero LLM cost over the nodes you write. The cost is **engineering time** — you write more code. The trade-off is: code complexity for control + reliability + HITL support.
...
- **Pattern reference:** [reference-agentic-patterns](reference-agentic-patterns.md) (Reflection / Plan-and-Execute / Supervisor — all custom-graph friendly)
