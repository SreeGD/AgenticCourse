...
| File | Role |
|---|---|
| [`29_memory_architectures.py`](../29_memory_architectures.py) | Five self-contained demos, one per architecture: FIFO-evicted working memory, vector-store episodic, entity-attribute fact store, learning-loop skill library, hot/warm/cold paging. ~280 lines, runs in ~30 s, costs ~$0.05. |
...
> **Session 17 (Claude Skills) was the deploy-time mechanism — author SKILL.md files yourself. This pattern is the *runtime learning loop* that grows the library from experience.** Combine them: hand-author core skills, let the loop add specialized variants.
...
```
cd labs
./.venv/bin/python 29_memory_architectures.py
```
...
└─ procedural ─┘  Claude Skills (author-written) + optional runtime growth
```
...
**Q: What's the relationship between Claude Skills (Session 17) and procedural memory here?**
Claude Skills are **author-written** procedural memory — SKILL.md files that ship with the agent. This lesson covers the **runtime learning** of new skills. Production stacks combine both: a baseline of hand-authored skills, plus a learning loop that adds specialized variants from successful runs.

**Q: How does hierarchical memory work with prompt caching (Session 4)?**
The `hot` tier is what gets sent to the LLM, so it's also what gets cached. Stable items in hot (long-tenured semantic facts) become cacheable prefix; the variable suffix is the new query + recent working memory. Anthropic's prompt cache + a smart hot tier is the LLM-era equivalent of L1 cache.
...
- **Builds on:** [08 — Chatbot Memory](08-chatbot-memory.md) (the MemorySaver baseline), [14 — Multi-agent + LTM](14-multi-agent-ltm.md) (semantic + episodic LTM), [17 — Claude Skills](17-claude-skills.md) (procedural memory as deployed skills)
