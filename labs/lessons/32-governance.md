...
| File | Role |
|---|---|
| [`32_governance.py`](../32_governance.py) | The minimum-viable governance plane: self-reported + meta-judge confidence scoring, rules-as-code policy engine (5 rules, first-match-wins), structured `GovernanceAuditEntry` per request, 5 demo scenarios exercising each rule, compliance framework mapping table. |
...
```
   ...
   USER REQUEST  ─────►   │  pre-flight (cheap)   │ ──── PII regex → if hit, DENY here
                          ...
                          └───────────┬───────────┘
                                      ...
                                      ▼
                          ┌───────────────────────┐
                          │   ANSWER + self_conf  │ ──── claude-sonnet-4-6
                          ...
                          └───────────┬───────────┘
                                      ...
                                      ▼
                          ┌───────────────────────┐
                          │  META-JUDGE confidence│ ──── claude-haiku-4-5
...
**Meta-judge confidence** comes from a *different* model (we use Haiku) that reads the question + answer + self-confidence and scores them independently. Has its own biases, but rarely the same biases as the answerer.
...
Saves the LLM call AND prevents the PII from being stored in the model provider's request logs. ~$0.005 per blocked request + a real privacy improvement.
...
```
cd labs
./.venv/bin/python 32_governance.py
```

~30-45 seconds (5 scenarios × 2 LLM calls each ≈ 10 calls; scenario 2 short-circuits at pre-flight so only 8 LLM calls actually fire). Cost ~$0.05.
...
- If gap suddenly jumps → upstream model change (provider updated their model silently)
...
**Q: Why two confidence numbers? Isn't that twice the cost?**
The meta-judge is Haiku (~$0.0002 per call). At 1M requests/month, meta-judging every one is ~$200/mo. The information you get — the gap as a calibration signal — is worth more than that in audit-readiness alone. For cost-sensitive deployments, sample (meta-judge 10% of requests, monitor drift).

**Q: Won't the meta-judge have its own biases?**
Yes. Mitigations: (1) use a *different model family* from the answerer when possible (less feasible at the moment since Anthropic is the only family in use), (2) periodically re-baseline the meta-judge against a human review panel, (3) for highest-stakes domains, use ensemble of 2-3 judges and vote.
...
`with_structured_output(AnswerWithConfidence)` returns a Pydantic instance with `answer` + `confidence` + `reasoning` populated in one call. No extra LLM round-trip. Modern Anthropic + LangChain make this clean.
...
Two numbers: (1) cost of an SOC 2 audit failure (six-figure consulting + delayed enterprise contracts), (2) cost of an AI-caused PR incident (varies, but recent precedents are seven figures + reputational damage). The governance plane costs ~$200/mo at 1M requests + a small engineering investment to set up. It pays for itself the first time it prevents one of those.
