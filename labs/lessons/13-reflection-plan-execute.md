...
| **Cost matters** — small model can plan, big model executes | ReAct uses one model for everything; you pay Opus rates for trivial sub-steps |
...
cost (Sonnet 4.6):  $0.0098           $0.0647
─────────────────────────────────────────────────────
                                  PE was 6.6× more expensive
                                  and 3.5× slower for this task
```

**But** PE produced a much more detailed, structured artifact — with FLOPs formulas, specific pricing numbers ($0.30/MTok cache vs $3.00/MTok standard), and prescriptive prompt-discipline patterns. The cost bought more depth.
...
Linear cost growth per iteration. Hard cap with `MAX_ITERATIONS` so a stubborn critic can't run up the bill.
...
| You can afford 2-5× cost for better quality | Reflection |
| You want predictable cost AND can run a cheap planner | Plan-and-Execute |
...
| Plan-and-Execute with cheap planner | Haiku for planning, Sonnet/Opus for execution — 50-70% cost reduction |
...
2. **Use Haiku for the planner** — switch the planner's model to Claude Haiku. Watch cost drop while quality holds.
...
A: The writer prompt was strong, the task was bounded, and the critic's criteria (accuracy + specificity + completeness + density) were satisfiable in one draft. **This is desired behavior** — Reflection only pays for iteration when iteration is needed. To force more iterations: tighten the rubric ("each paragraph must include 2+ specific numbers"; "each claim must cite a source") and re-run.

**Q: Should the writer and critic use different models?**

A: Yes, ideally. Same-model critique tends toward agreement (the critic shares the writer's blind spots). Use a different model family or at least different system prompts. In production, **Sonnet for writer + Opus for critic** (or vice versa) breaks the agreement bias.
...
A: 3-5 for most tasks. Beyond 5, you're usually fighting a too-strict critic rather than improving the draft. **Hard cap is non-optional** — without it a critic can loop forever and run up your bill.
...
A: When iterations converge quickly. Reflection at 1 iteration = 2 LLM calls. PE at N=4 steps = 6 LLM calls. So if Reflection converges in ≤2 iterations, it's cheaper than PE with 4 steps. Beyond 2 iterations, PE wins on cost.
