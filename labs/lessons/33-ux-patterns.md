...
```
cd labs
./.venv/bin/python 33_ux_audit_helper.py
```
...
A: Make the bar simple: any feature gated by a leadership review must come with the audit attached. Once the audit is required for sign-off, teams will run it. The barrier is usually social, not technical — the tool is `./.venv/bin/python 33_ux_audit_helper.py` and a string.
