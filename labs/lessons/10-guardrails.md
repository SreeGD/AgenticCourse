...
- A user asks off-topic things → wasted LLM cost
...
1. **Reorder guards** — move the on-topic judge *before* PII regex. Watch LLM costs increase as adversarial inputs hit the LLM before the cheap regex catches them.
...
A: Regex catches the basic ones. For depth, use **Rebuff** (multi-layer: heuristics + canary tokens + LLM judge) or **NeMo Guardrails** (Colang DSL for conversational flows). For high-stakes apps, also do red-team testing (see Architect Skills track).
...
- **Use a smaller model for guards** — Haiku for guards, Sonnet for main work; ~80% cost reduction
...
- **Next:** [11 — Production capstone](11-production-capstone.md) (RAG + memory + caching + guardrails composed)
