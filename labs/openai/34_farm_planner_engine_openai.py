"""Farm planner engine (OpenAI variant) — pure Python, no UI imports.
...
_raw_openai_client = _openai.OpenAI()

CHECKPOINT_DB_PATH = HERE / "farm_plans" / "checkpoints_openai.sqlite"
...
def _call_openai_structured(
    ...
    """Call OpenAI API with structured output via JSON schema.
    ...
    for attempt in range(max_retries):
        if attempt > 0:
            ...
            print(f"[openai] {label}: backing off {wait}s before retry {attempt + 1}...")
            time.sleep(wait)
        try:
            response = _raw_openai_client.chat.completions.create(
                model=model,
            ...
            print(f"[openai] {label}: prompt={usage.prompt_tokens} "
                  f"cached={cached} "
            ...
            last_err = e
            if attempt < max_retries - 1:
                print(f"[openai] {label}: {type(e).__name__} on attempt "
...
def _node_crop_intent(state: PlanGraphState) -> dict:
    """Tiny LLM call that just NAMES the crops to include (3-6 of them)."""
    profile = state["profile"]
    goals = state["goals"]
...
def _generate_one_crop_detail(ci: CropIntentItem, profile: FarmProfile,
    ...
    """Generate ONE CropInPlan in detail."""
    prompt = (
        f"{_risk_profile_instruction(risk_profile)}\n\n"
        ...
        f"variety-level specificity. Use the KB for variety, supplier, scheme references."
    )
...
def _node_crops_parallel(state: PlanGraphState) -> dict:
    """Run all crop_detail calls in PARALLEL via a thread pool."""
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as pool:
        ...
        for future in as_completed(futures):
            ...
            try:
                crop = future.result()
                crops.append(crop)
                print(f"[engine] crops_parallel: done {ci.crop_name}")
            except Exception as e:
...
def build_planner_graph(checkpointer=None):
    ...
    g.add_node("crops_parallel", _node_crops_parallel)
...
def generate_three_options(profile: FarmProfile,
    ...
    """Generate 3 plans — conservative + balanced + aggressive — plus a recommendation pick."""
    options: list[PlanOption] = []
...
def render_plan_markdown(plan: FarmPlan, score: SustainabilityScore | None = None) -> str:
    ...
    if plan.livestock:
        ...
        for l in plan.livestock:
            lines.append(f"\n### {l.type.replace('_', ' ').title()} — {l.breed} x {l.count}")
...
if __name__ == "__main__":
    print("Farm planner engine (OpenAI variant) - module sanity check")
